...
if [[ $# -ne 4 ]]; then
    echo "Usage: $0 INS STATE_DIR SOIL_CODES OUTPUT_DIR" >&2
    exit 2
fi
...
probe=$script_dir/bin/restart_state_probe
readback_shards=${LPJG_READBACK_SHARDS:-16}
parallel_jobs=${LPJG_PARALLEL_JOBS:-16}

for path in "$ins" "$state/meta.bin" "$soil_codes" "$probe"; do
...
for ((first = 0; first < readback_shards; first += parallel_jobs)); do
    ...
    for shard in $(seq "$first" "$last"); do
        "$probe" "$ins" "$state" "$soil_codes" "$shard" "$readback_shards" \
            "$output/shard_${shard}.tsv" >"$output/shard_${shard}.log" 2>&1 &
        pids+=("$!")
    done
    failed=0
    for pid in "${pids[@]}"; do wait "$pid" || failed=1; done
    if (( failed )); then
        echo "State read-back failed in shard batch $first-$last" >&2
        exit 1
    fi
done
...
[[ $count -eq $readback_shards ]] || {
    echo "Incomplete state read-back: expected $readback_shards tables, found $count" >&2
    exit 1
}
echo "State read-back complete: $output"
